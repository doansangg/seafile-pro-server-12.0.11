...
from seafes.indexes.repo_file.file_index_updater import FileIndexUpdater
...
class IndexWorker(object):
    ...
    def update_repo(self, mq, repo_id):
        commit_id, repo_type = repo_data.get_repo_head_commit(repo_id)
        if repo_type == REPO_TYPE_WIKI:
            return
        if not commit_id:
