...
from seafes.indexes import FileIndexUpdater
from seafes.repo_data import repo_data
from seafes.constants import REPO_TYPE_WIKI
        ...
        while True:
            try:
                ...
                try:
                    ...
                    logger.exception('Index Repo Error: %s, repo_id: %s' % (e, repo_id), exc_info=True)
                    self.incr_error()
...
def delete_indices():
    es = es_get_conn()
    for idx in (seafes_config.repo_status_index_name, seafes_config.repo_files_index_name):
